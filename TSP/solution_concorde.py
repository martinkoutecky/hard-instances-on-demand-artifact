...
def solve(dist_matrix, n, seed=0):
    from concorde._concorde import _CCutil_gettsplib, _CCtsp_solve_dat

    int_dist = np.round(np.asarray(dist_matrix)).astype(int)

    old_cwd = os.getcwd()
    scratch_dir = tempfile.mkdtemp(prefix="tsp_concorde_")
    try:
