...
# BLAS thread caps must be set BEFORE any numpy import — otherwise per-worker
# oversubscription destroys multiprocessing scaling.
...
def gen(dct):
    ...
    if "timer" in dct:
        from solution import set_timer
        set_timer(dct["timer"])

    test = My_Test(
        dct["N"], dct["M"], dct["U"],
        solver=dct.get("solver", "cpsat"),
